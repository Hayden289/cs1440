...
void RegionTester::testSubRegions()
{
...

    {
        ...
        Region* subregion = world->findSubregion(25);
        if (subregion->getId() != 25) {
        ...
        std::string inputString = "Tiny Country,30,20";
        Region *region = Region::create(Region::NationType,inputString);
        world->addSubregion(region);
        if (world->findSubregion(region->getId()) != region) {
            std::cout << "Failed to add new region from " << inputString << std::endl;
            std::cout << "\tExpected region 28, but got \"" << region->getId() << "\"\n";
            return;
        }

        if (!world->removeSubregion(region->getId())) {
            std::cout << "Failed to remove region by id\n";
            std::cout << "\tExpected true, but got \"false\"\n";
            return;
        }

        if (world->getSubRegionCount() != 12) {
            std::cout << "Failed to get the correct sub-region count from\n" << inputFile << std::endl;
            std::cout << "\tExpected 2, but got \"" << world->getSubRegionCount() << "\"\n";
            return;
        }
    }
}
...
void RegionTester::testComputeTotalPopulation()
{
...

    {
        ...
        if (world->computeTotalPopulation() != 4059243) {
            std::cout << "Failed to compute total population from " << inputFile << std::endl;
            std::cout << "\tExpected 4059243 but got \"" << world->computeTotalPopulation() << "\"\n";
            return;
        }
    }
}
